Shader "Hidden/Universal Render Pipeline/UberPost"
{
    HLSLINCLUDE
        ...
        #pragma multi_compile_local_fragment _ _VRS_LUMA_OUTPUT
        ...
        #if _VRS_LUMA_OUTPUT
        RWTexture2D<uint> _LumGradUAV : register(u1);   // 半分辨率 (w+1)/2 × (h+1)/2，R32_UInt
        ...
        half4 FragUberPost(Varyings input) : SV_Target
        {
            ...
            half L = VRSLuminance(color);                   // 感知亮度（= DF 的 GetLuminanceFP16(OutColor) 语义）

            float H = QuadReadAcrossX(L);          // 水平邻亮度
            ...
            if (((ScreenPos.x & 1) == 0) && ((ScreenPos.y & 1) == 0))
            {
                ...
                _LumGradUAV[ScreenPos >> 1] = qL | (qH << 8) | (qV << 16) | (qD << 24);
            }
            #endif
