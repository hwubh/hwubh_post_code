...
namespace UnityEngine.Rendering.Universal
{
    ...
    public class PostProcessPass : ScriptableRenderPass
    {
        ...
        // [VRS] 亮度梯度图已移至 UniversalRenderer.luminanceRT（实例成员），
        // PostProcessPass 经 cameraData.renderer 访问；不再用 static（域重载/多 renderer 泄漏）
        ...
        void Render(CommandBuffer cmd, ref RenderingData renderingData)
        {
            ...
            using (new ProfilingScope(cmd, ProfilingSampler.Get(URPProfileId.UberPostProcess)))
            {
                ...
                {
                    // [VRS] RT 由 renderer 持有（生命周期/释放归 renderer），此处只做尺寸检查与重分配
