...
float2 PackNormalOctQuadEncode(float3 n)
{
    ...
    // Use sign() to reduce branching - may be faster on some GPUs
    // Note: sign(0) returns 0, but n.x/n.y should rarely be exactly 0 after normalization
    float2 signXY = sign(n.xy);
    return n.xy + signXY * t;
}
...
// Optimized version that directly outputs [0, 1] range
// This eliminates the need for external * 0.5 + 0.5 and saturate operations
// Usage: PackNormalOctQuadEncode01(n) instead of saturate(PackNormalOctQuadEncode(n) * 0.5 + 0.5)
float2 PackNormalOctQuadEncode01(float3 n)
{
    n *= rcp(max(dot(abs(n), 1.0), 1e-6));
    float t = saturate(-n.z);
    // Use sign() to reduce branching
    float2 signXY = sign(n.xy);
    float2 result = n.xy + signXY * t;
    // Use mad (multiply-add) for better performance on some GPUs
    return saturate(mad(result, 0.5, 0.5));
}

// Low-end device optimized version using real (half on mobile) precision
// Assumes input vector is already normalized (common case for reflection vectors)
// For best performance on low-end devices, use this version
real2 PackNormalOctQuadEncode01_LowEnd(real3 n)
{
    // Fast path: assume normalized input (most reflection vectors are normalized)
    // If not normalized, use PackNormalOctQuadEncode01() instead
    real l1norm = dot(abs(n), 1.0);
    real invL1Norm = rcp(max(l1norm, 1e-6));
    n *= invL1Norm;
    
    real t = saturate(-n.z);
    // Avoid sign() on low-end devices - use conditional which may be faster on some GPUs
    real2 result = n.xy + real2((n.x >= 0.0) ? t : -t, (n.y >= 0.0) ? t : -t);
    
    // Direct calculation instead of mad for better compiler optimization
    return saturate(result * 0.5 + 0.5);
}

// Ultra-optimized inline version for low-end devices
// Use this when you know the vector is normalized and want maximum performance
// Inline this code directly in your shader to avoid function call overhead
// Usage example:
//   real l1norm = dot(abs(reflectVector1), 1.0);
//   real invL1Norm = rcp(max(l1norm, 1e-6));
//   real3 n = reflectVector1 * invL1Norm;
//   real t = saturate(-n.z);
//   real2 octUV = saturate((n.xy + real2((n.x >= 0.0) ? t : -t, (n.y >= 0.0) ? t : -t)) * 0.5 + 0.5);
