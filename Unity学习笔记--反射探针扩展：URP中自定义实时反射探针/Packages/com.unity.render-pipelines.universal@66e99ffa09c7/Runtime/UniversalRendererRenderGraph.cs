...
namespace UnityEngine.Rendering.Universal
{
    ...
    public sealed partial class UniversalRenderer
    {
        ...
        bool RequiresIntermediateAttachments(UniversalCameraData cameraData, in RenderPassInputSummary renderPassInputs, bool requireCopyFromDepth, bool applyPostProcessing)
        {
            ...
            if(frameData.Contains<UniversalReflectionProbeData>())
            {
                var probeData = frameData.Get<UniversalReflectionProbeData>().probeData;
                if (probeData != null && !probeData.finalBlitPass)
                    isOffScreenRendering = true;
            }

            var requireColorTexture = !isOffScreenRendering && HasActiveRenderFeatures(rendererFeatures) && m_IntermediateTextureMode == IntermediateTextureMode.Always;
        ...
        private void OnAfterRendering(RenderGraph renderGraph, bool applyPostProcessing)
        {
            ...
            // 检查反射探针数据，如果未启用 Draw Gizmos Pass，则不绘制 Gizmos
            if(frameData.Contains<UniversalReflectionProbeData>())
            {
                var probeData = frameData.Get<UniversalReflectionProbeData>()?.probeData;
                if (probeData != null && !probeData.drawGizmosPass)
                    drawGizmos = false;
            }
